...
## 简介
本文件面向运营中心模块，系统化阐述经销商管理、产品线管理、用户范围控制以及新增的订单管理功能的业务与技术实现，重点覆盖：
...
## 项目结构
运营中心模块位于 yudao-module-opshub，采用"控制器-服务-持久层-数据对象-枚举-权限配置"的分层组织方式，围绕"经销商""产品线""用户范围""订单"四大领域展开。
...
## 架构总览
运营中心模块遵循标准分层架构，前端通过控制器暴露 REST 接口，服务层编排业务，持久层访问数据库；数据权限规则在 SQL 查询阶段自动注入 WHERE 条件，确保按角色维度进行数据隔离。新增的订单管理模块与现有模块保持一致的架构风格。
...
- 字典与数据标准化
  ...
  - 经销商状态、产品线状态、订单状态等统一使用字典值，保证前后端一致性
- 集成建议
...
- 查询优化
  ...
  - 合理设置索引：dealer_code、product_line_code、order_code、tenant_id 等常用过滤字段
...
## 结论
运营中心模块通过清晰的分层设计与完善的数据权限机制，实现了经销商、产品线、用户范围以及新增订单管理的精细化管理。结合多租户与角色维度的访问控制，既能满足业务灵活性，又能保障数据安全与合规。新增的订单管理模块进一步完善了运营中心的业务闭环，为订单全生命周期管理提供了完整的技术支撑。

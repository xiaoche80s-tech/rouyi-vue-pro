...
- [2026-06-19_150000.sql](file://db/branches/feature_step1-权限功能设计/history/2026-06-19_150000.sql)
- [PRD-用户权限设计.md](file://docs/PRD-用户权限设计.md)
</cite>

## 更新摘要
**变更内容**
- 更新菜单名称从'经销商管理 SaaS'重命名为'聚院通'
- 更新数据库脚本中的菜单名称变更记录
- 更新环境配置中的应用标题
- 更新PRD文档中的菜单名称引用
...
**更新** 菜单名称已从'经销商管理 SaaS'重命名为'聚院通'，影响所有相关权限和角色配置。
...
- 前端
  ...
  - 应用配置：src/config/axios/config.ts
- 后端
...
- 数据库脚本
  - 菜单数据初始化：db/branches/feature_step1-权限功能设计/feature_step1-权限功能设计_dml.sql
  - 菜单重命名历史记录：db/branches/feature_step1-权限功能设计/history/2026-06-19_150000.sql
...
DB_SCRIPT["数据库脚本<br/>feature_step1-权限功能设计_dml.sql"]
DB_HISTORY["历史记录<br/>2026-06-19_150000.sql"]
ENV_CONFIG["环境配置<br/>docker.env / docker-compose.yml"]
...
- [docker.env:19](file://script/docker/docker.env#L19)
- [docker-compose.yml:66](file://script/docker/docker-compose.yml#L66)
- [feature_step1-权限功能设计_dml.sql:25](file://db/branches/feature_step1-权限功能设计/feature_step1-权限功能设计_dml.sql#L25)
- [2026-06-19_150000.sql:3](file://db/branches/feature_step1-权限功能设计/history/2026-06-19_150000.sql#L3)
...
- 环境配置：应用标题、API基础路径等运行时配置。

**更新** 环境配置中应用标题已更新为'聚院通'，确保前后端一致性。
...
### 数据库脚本与菜单重命名
- 菜单数据初始化：一级目录'聚院通'（原'经销商管理 SaaS'）包含9个二级菜单和43个按钮权限
- 菜单重命名历史：通过UPDATE语句将一级菜单名称从'经销商管理 SaaS'重命名为'聚院通'

**更新** 数据库脚本已反映菜单名称变更，确保数据库与前端配置的一致性。
...
- 前端
  ...
  - 环境配置依赖 docker 环境变量，确保应用标题一致性。
- 后端
  - 菜单服务依赖类型枚举与 VO，按类型初始化属性。
  - 控制器 VO 定义了菜单的请求/响应字段约束。
- 数据库
  - 菜单数据依赖脚本初始化，包含完整的菜单树结构和权限配置。
...
- 环境配置
  - 应用标题统一配置，避免多处硬编码导致的维护成本。

**更新** 环境配置集中管理，确保应用标题一致性。
...
- 菜单名称不一致
  - 检查数据库脚本是否正确执行，确认一级菜单名称已更新为'聚院通'。
  - 验证环境配置中的应用标题是否与数据库一致。
...
- [routerHelper.ts:73-267](file://yudao-ui/yudao-ui-admin-vue3/src/utils/routerHelper.ts#L73-L267)
- [index.ts:21-49](file://yudao-ui/yudao-ui-admin-vue3/src/api/system/menu/index.ts#L21-L49)
- [feature_step1-权限功能设计_dml.sql:189-191](file://db/branches/feature_step1-权限功能设计/feature_step1-权限功能设计_dml.sql#L189-L191)
...
**更新** 菜单名称已统一为'聚院通'，数据库脚本、环境配置和PRD文档均已相应更新，确保系统各组件间的一致性和准确性。
...
### 环境配置与应用标题
- 应用标题：统一配置为'聚院通'，确保前后端一致性。
- API基础路径：配置为'/prod-api'，支持生产环境部署。
- 多语言支持：通过i18n框架支持国际化。

**更新** 环境配置已更新为'聚院通'应用标题，确保系统整体一致性。

**章节来源**
- [docker.env:19](file://script/docker/docker.env#L19)
- [docker-compose.yml:66](file://script/docker/docker-compose.yml#L66)
- [config.ts:10](file://yudao-ui/yudao-ui-admin-vue3/src/config/axios/config.ts#L10)

### 菜单重命名历史
- 菜单名称变更：从'经销商管理 SaaS'重命名为'聚院通'
- 影响范围：数据库脚本、前端配置、PRD文档
- 执行时间：2026-06-19 15:00:00

**更新** 新增菜单重命名历史记录，便于追溯变更过程。

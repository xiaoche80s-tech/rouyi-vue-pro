...
## 更新摘要
**变更内容**
- 新增 DealerSelectDialog 经销商选择对话框组件，支持多经销商场景下的智能选择
- 新增 useCsConsult 咨询共享组合式函数，统一管理咨询会话创建与经销商选择逻辑
- 增强 AssignTaskModal 工单下发模态框，支持紧急程度、SLA截止时间等高级功能
- 新增 CsExecutorNotifier 执行员通知组件，提供新咨询实时推送功能
- 完善聊天组件系统架构，提升用户体验和功能完整性
...
- **AssignTaskModal**：工单下发模态框，支持紧急程度和SLA截止时间设置
- **DealerSelectDialog**：经销商选择对话框，处理多经销商场景
- **CsExecutorNotifier**：执行员通知组件，提供新咨询实时推送
...
CW --> ATM["AssignTaskModal<br/>工单下发"]
CW --> DSD["DealerSelectDialog<br/>经销商选择"]
CEN["CsExecutorNotifier<br/>执行员通知"] --> WS["WebSocket Hook"]
```
...
#### useCsConsult 咨询组合式函数
useCsConsult 提供统一的咨询管理能力，简化各业务模块的集成：

- **单条咨询**：openConsult 直接创建会话并打开聊天窗口
- **批量咨询**：openBatchConsult 支持最多20条记录的并行创建
- **分类入口**：openByCategory 根据用户授权经销商数量智能决策
- **经销商选择**：onDealerSelected 处理多经销商场景的选择回调
- **状态管理**：统一管理 chatVisible、currentSessionId、dealerDialogVisible 等状态

```mermaid
flowchart TD
Start["调用 openByCategory"] --> Check["检查用户授权经销商数量"]
Check --> Zero{"0 个经销商?"}
Zero --> |是| NoAuth["提示无授权"] --> End
Zero --> |否| One{"1 个经销商?"}
One --> |是| DirectCreate["直接创建会话"] --> OpenChat["打开聊天窗口"] --> End
One --> |否| Multi["多个经销商"] --> ShowDialog["显示选择对话框"] --> Select["用户选择"] --> Create["创建会话"] --> OpenChat
End
```
...
#### 客服 API 集成
csSession 和 dealerScope API 提供完整的客服聊天功能接口：

- **会话管理**：getSession、acceptSession、completeSession、closeSession、createSession
- **消息处理**：sendMessage、getMessagePage、markRead
- **统计查询**：getSessionStatistics、getSessionPage
- **经销商授权**：getMyDealers、assignDealerScope、getDealerScopeUsers
- **数据模型**：CsSessionVO、CsMessageVO、DealerItemVO、CsStatisticsVO 类型定义
...
## 结论
本架构以 Vue 3 + TypeScript 为基础，结合 Composition API、Pinia、Vue Router、Element Plus 与 Axios，形成清晰的职责边界与可扩展的模块化结构。通过完善的权限体系、国际化与主题定制、网络层封装与构建优化，能够支撑复杂后台系统的前端需求。新增的聊天组件系统进一步增强了实时通信能力，通过 WebSocket 钩子、useCsConsult 组合式函数和完整的组件族，为客服咨询场景提供了专业的一站式解决方案。建议在后续迭代中持续完善组件库生态、增强可访问性与自动化测试，进一步提升开发效率与用户体验。

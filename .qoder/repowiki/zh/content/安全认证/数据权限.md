...
## 更新摘要
**变更内容**
- 新增客户服务工单的assignee绕过机制，允许分配处理人始终能看到其分配的任务
- 增强跨产品线可见性修复功能，解决执行员无法查看跨产品线分配工单的问题
- 扩展数据权限规则的灵活性，支持按表粒度的特殊权限处理
...
- **新增**：处理人旁路机制：为特定业务表提供assignee绕过权限，确保分配处理人始终能看到其分配的任务
...
N["处理人旁路机制<br/>assigneeBypassTables"]
O["旁路表注册<br/>addAssigneeBypass()"]
end
A --> B
B --> G
G --> H
...
## 架构总览
下图展示从注解到 SQL 合成的完整链路，以及与通用权限 API 的交互，包括新增的处理人旁路机制。
...
participant R as "规则实现<br/>DealerDataPermissionRule"
...
R->>API : 查询当前用户部门数据权限
API-->>R : 返回权限结果
R->>R : 检查是否为旁路表assigneeBypassTables
alt 是旁路表
R->>R : 构建原始条件 + OR assignee_id = currentUserId
else 非旁路表
R-->>R : 仅构建原始条件
end
R-->>RH : 返回 SQL 片段
...
- **新增**：处理人旁路机制：为特定业务表注册旁路，确保分配处理人始终能看到其分配的任务
...
### 处理人旁路机制（新增功能）
- **核心功能**：为特定业务表提供assignee绕过权限，确保分配处理人始终能看到其分配的任务
- **实现原理**：
...
- **应用场景**：解决跨产品线分配场景下的数据可见性Bug，确保执行员能够查看分配给自己的工单
- **安全考虑**：旁路仅在表注册了`addAssigneeBypass`时生效，不影响其他表；`super_admin`角色返回null（不过滤），旁路不生效
...
RF --> DLR["经销商规则<br/>DealerDataPermissionRule.java"]
...
DL --> ABT["旁路表集合<br/>assigneeBypassTables"]
ABT --> DLR
```
...
- **新增**：旁路表集合查找：使用HashSet存储旁路表名，查找时间复杂度为O(1)，对性能影响最小
...
- 症状：查询结果为空或越权
  ...
  - **新增**：检查旁路表注册是否正确，确认`assigneeBypassTables`集合中是否包含目标表
- 症状：SQL 报错或执行异常
  - 检查规则生成的 SQL 片段是否与现有查询条件冲突
  - 确认字段名与表名大小写及转义是否一致
  - **新增**：检查旁路条件的括号包裹是否正确，避免SQL优先级问题
- 症状：性能下降
  - 关注规则数量与 SQL 合并复杂度
  - 检查是否存在不必要的全表扫描
  - **新增**：旁路表数量控制，避免旁路表过多影响查找性能
- 调试建议
  - 开启相关日志，观察规则解析与 SQL 合成过程
  - 使用单元测试覆盖关键分支（如"查看全部/仅自己/指定部门集合"）
  - **新增**：测试旁路机制，验证分配处理人能否正确查看跨权限工单
  - 提供权限测试工具，模拟不同角色与部门范围下的查询结果

## 结论
数据权限模块通过注解驱动、AOP 拦截与规则工厂实现了灵活、可扩展的动态过滤能力。部门规则与业务扩展规则（如经销商/产品线）可按需组合，既保证了多层防护，又兼顾了性能与可维护性。**新增的处理人旁路机制进一步增强了系统的实用性，特别适用于客户服务工单等需要跨权限协作的业务场景。** 建议在实际落地时结合业务场景完善规则配置与测试流程，并持续关注日志与性能指标。

## 附录
- 规则可视化管理与测试工具（概念性说明）
  - 规则编辑器：提供图形化界面配置表与字段映射、角色与维度的对应关系
  - **新增**：旁路表管理：支持按业务表粒度启用或禁用处理人旁路功能

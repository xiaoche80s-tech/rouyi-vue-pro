<template>
  ...
  <ContentWrap>
    <div class="mb-10px flex items-center gap-10px">
      <el-button
        v-hasPermi="['dealer:cs-task:create']"
        type="primary"
        plain
        @click="handleCreate"
      >
    ...
    <el-table v-loading="loading" :data="list" border stripe>
      ...
      <el-table-column label="经销商" prop="dealerCode" min-width="100" />
      ...
      <el-table-column label="操作" fixed="right" min-width="240" align="center">
        <template #default="{ row }">
          ...
          <el-button
            v-if="row.status === 0"
            v-hasPermi="['dealer:cs-task:accept']"
            link type="primary"
          ...
          <template v-if="row.status === 1">
            <el-button
              v-hasPermi="['dealer:cs-task:deliver']"
              link type="success"
              @click="handleDeliver(row)"
            >交付</el-button>
            <el-button
              v-hasPermi="['dealer:cs-task:transfer']"
              link type="warning"
          ...
          <template v-if="row.status === 2">
            <el-button
              v-hasPermi="['dealer:cs-task:verify']"
              link type="success"
          ...
          <el-button
            v-if="row.status === 4"
            v-hasPermi="['dealer:cs-task:reprocess']"
            link type="primary"
          ...
          <el-button
            v-if="row.status !== 3"
            v-hasPermi="['dealer:cs-task:urge']"
            link type="info"
...
const handleAccept = async (row: any) => {
  ...
  await CsTaskApi.acceptTask(row.id)
...
const handleDeliver = async (row: any) => {
  ...
  await CsTaskApi.deliverTask(row.id)
...
const submitTransfer = async () => {
  ...
  try {
    await CsTaskApi.transferTask(transferForm)
...
const handleVerify = async (row: any, passed: boolean) => {
  ...
  await CsTaskApi.verifyTask({ id: row.id, passed: true })
...
const handleReprocess = async (row: any) => {
  ...
  await CsTaskApi.reprocessTask(row.id)
...
const handleUrge = async (row: any) => {
  await CsTaskApi.urgeTask(row.id)
